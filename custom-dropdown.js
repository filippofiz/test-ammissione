// Custom Dropdown Component con supporto per colori e categorie
class CustomDropdown {
  constructor(container, options = {}) {
    this.container = container;
    this.value = options.value || '';
    this.placeholder = options.placeholder || 'Seleziona...';
  ...
  init() {
    ...
    // Aggiungi stili CSS se non già presenti
    this.addStyles();
    
    // Bind eventi
    this.bindEvents();
    
    // Render items iniziali
    this.renderItems();
  }
  ...
  bindEvents() {
    // Click su input per aprire/chiudere
    ...
    this.input.addEventListener('blur', (e) => {
      // Delay per permettere click su items
      setTimeout(() => {
        if (!this.wrapper.contains(document.activeElement)) {
          this.close();
          // Se vuoto, ripristina valore precedente
  ...
  selectItem(item) {
    ...
    this.validate(item.value); // Rimuove il rosso quando selezioni un valore valido
    this.close();
  ...
  validate(value) {
    ...
    if (!isValid && trimmedValue !== '') {
      ...
      this.input.title = '⚠️ Valore non valido! Seleziona un\'opzione dalla lista.';
    } else {
