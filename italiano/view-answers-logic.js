...
function displayQuestionsAndAnswers(questionsData, studentAnswersMap) {
  ...
  if (isSATTest) {
    ...
    // Display questions grouped by module - Keep module names in English
    const moduleNames = {
    ...
    if (notShownQuestions.length > 0) {
      ...
      notShownSection.innerHTML = `
        <div class="sat-section-header not-shown-header">
          <h3>📝 Moduli Adattivi Non Mostrati (${notShownQuestions.length})</h3>
          <button id="toggleNotShown" class="toggle-btn">Mostra ▼</button>
        </div>
        <div id="notShownContainer" class="not-shown-container" style="display: none;">
          <p class="info-text">Queste domande non sono state mostrate allo studente in base al percorso adattivo.</p>
        </div>
      `;
...
function displaySingleQuestion(q, studentAnswerRaw, questionNumber, isNotShown, targetContainer = null) {
    ...
    header.innerHTML = `
      <span class="question-label" data-number="${questionNumber}">Domanda ${questionNumber}</span>
      ${mark}
    `;
