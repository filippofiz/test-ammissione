/**
 * GMAT Material Page
 * Student and tutor view for GMAT study materials
 *
 * Features:
 * - Materials grouped by section > topic > material_type
 * - PDF viewer modal
 * - Tutor view: See all materials + lock/unlock controls
 * - Student view: Only sees unlocked materials (hidden: Assessments, Context, Slides)
 */
...
export default function GMATMaterialPage() {
  ...
  // Determine if this is tutor view
  ...
  async function loadData() {
    ...
    try {
      ...
      // If tutor view, fetch student info
      if (studentId) {
      ...
      // Auto-expand reference section (always visible for students)
      const allSections = new Set<string>();
      materialsWithStatus.forEach(m => allSections.add(m.section));
      if (allSections.has('reference')) {
        setExpandedSections(new Set(['reference']));
      }
  ...
  function groupMaterials(materials: MaterialWithStatus[]): GroupedMaterials {
    ...
    for (const material of materials) {
      // Always filter out templates (admin/allocation use only)
      ...
      if (!isTutorView) {
        ...
        if (material.section === 'Assessments' || material.section === 'Slides') {
          continue;
        }

        // Context section: only show 'overview' material type
        if (material.section === 'Context' && material.material_type !== 'overview') {
          continue;
        }

        // Only show unlocked materials (except reference section which is always visible)
        if (material.section !== 'reference' && !material.isUnlocked) {
          continue;
        }
      }
  ...
  async function viewPdf(material: MaterialWithStatus) {
    try {
      ...
      if (effectiveStudentId && !material.isViewed) {
        await supabase
          .from('2V_material_assignments')
  ...
  async function unlockMaterial(material: MaterialWithStatus) {
    if (!effectiveStudentId || !currentUser?.id) return;
  ...
  const pageTitle = isTutorView
    ? `GMAT Materials - ${student?.name || 'Student'}`
    : 'GMAT Study Materials';
  const pageSubtitle = isTutorView
    ? student?.email || ''
      ...
      <div className="flex-1 p-4 md:p-8">
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <span>Back to GMAT Preparation</span>
          </button>
          ...
          {!hasVisibleMaterials && (
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-gray-600 mb-2">
                {isTutorView ? 'No materials available' : 'No materials unlocked yet'}
              </h3>
            ...
            return (
              <div key={section} className="mb-6">
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between p-4 rounded-lg border-2 ${colors.border} ${colors.bg} hover:opacity-90 transition-all`}
                >
                ...
                {isExpanded && (
                  <div className="mt-2 ml-4 space-y-3">
                    {topics.map(topic => {
                      ...
                      return (
                        <div key={topicKey} className="bg-white rounded-lg border border-gray-200">
                          {/* Topic Header */}
                          <button
                              ...
                              {topicMaterials.map(material => (
                                <div
                                  ...
                                  className={`p-3 flex items-center justify-between ${
                                    !material.isUnlocked ? 'bg-gray-50 opacity-70' : ''
                                  }`}
                                >
                                  <div className="flex items-center gap-3">
                                    <FontAwesomeIcon
