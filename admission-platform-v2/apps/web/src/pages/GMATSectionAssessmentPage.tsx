/**
 * GMAT Section Assessment Page
 * Specialized page for taking GMAT section assessments (QR, DI, VR)
 * - Fetches questions from the GMAT pool based on student's cycle
 * - Uses IRT-based GMAT section scoring (60-90 range)
...
// Map short section codes to full names used by the scoring algorithm
type GMATSectionFullName = 'Quantitative Reasoning' | 'Verbal Reasoning' | 'Data Insights';
const SECTION_CODE_TO_FULL: Record<GmatSection, GMATSectionFullName> = {
  QR: 'Quantitative Reasoning',
  DI: 'Data Insights',
  VR: 'Verbal Reasoning',
};

export default function GMATSectionAssessmentPage() {
  const { section: sectionParam } = useParams<{ section: string }>();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();

  // Validate and normalize section param
  const section = (sectionParam?.toUpperCase() || '') as GmatSection;
  const isValidSection = ['QR', 'DI', 'VR'].includes(section);
  const sectionConfig = isValidSection ? SECTION_ASSESSMENT_CONFIG[section] : null;
  ...
  const [gmatSectionScore, setGmatSectionScore] = useState<number | null>(null);
  const [gmatPercentile, setGmatPercentile] = useState<number | null>(null);
  ...
  const testProgress = useTestProgress({
    sessionId: `section-assessment-${section}`,
    userId: studentId || '',
    storageKeyPrefix: 'gmat_section_assessment_',
    disabled: isPreviewMode || !isValidSection || !studentId,
    testType: 'gmat_section_assessment',
  });
  ...
  async function loadData() {
    if (!isValidSection || !sectionConfig) {
      setError(`Invalid section: "${sectionParam}". Must be QR, DI, or VR.`);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (isPreviewMode) {
        // Preview mode: use Development cycle as default
        setStudentCycle('Development');

        const { questions: selectedRefs, error: selectionError } =
          await getSectionAssessmentQuestions(profile.id, section, 'Development');

        if (selectionError || selectedRefs.length === 0) {
          setError(selectionError || 'No questions available for preview');
          setLoading(false);
          return;
        }

        // Fetch full question data
        const questionIds = selectedRefs.map(q => q.id);
        ...
        // Preserve randomized order from getSectionAssessmentQuestions
        ...
        setInitialTimeSeconds(sectionConfig.timeMinutes * 60);
      } else {
        // Student mode: require assigned cycle
        ...
        const lockKey = `section_${section.toLowerCase()}_locked` as keyof typeof progress;
        if (progress[lockKey]) {
          setError('This section assessment is currently locked by your tutor.');
          setLoading(false);
          return;
        }
        ...
        const { questions: selectedRefs, error: selectionError } =
          await getSectionAssessmentQuestions(profile.id, section);

        if (selectionError || selectedRefs.length === 0) {
          setError(selectionError || `No questions available for ${sectionConfig.fullName}. Please contact your tutor.`);
          setLoading(false);
          return;
        }
      ...
      setError(err instanceof Error ? err.message : 'Failed to load section assessment');
    } finally {
      setLoading(false);
    }
  }
  ...
  function goToQuestion(index: number) {
    ...
    if (currentQ) {
      const additionalTime = Math.round((Date.now() - questionStartTime) / 1000);
      const existingAnswer = answers.get(currentQ.id);
      if (existingAnswer) {
  ...
  function nextQuestion() {
    const currentQ = questions[currentIndex];
    const hasAnswer = currentQ && answers.has(currentQ.id);
  ...
  async function submitTest() {
    if (submitting || !studentId || !sectionConfig) return;
    ...
    try {
      ...
      for (const question of questions) {
        ...
        if (!userAnswer || userAnswer.answer === '__UNANSWERED__') {
          ...
          // Don't include unanswered in IRT response pattern — they're handled via penalty
          continue;
        }
        ...
        // Get DI subtype for proper answer comparison and guessing parameter
      ...
      const sectionFullName = SECTION_CODE_TO_FULL[section];
      const percentile = getSectionPercentile(sectionFullName, sectionScore);

      // Merge per-item IRT details with question IDs (responsePattern and perItemDetails are both ordered by answered questions)
      const responsePatternDetails = perItemDetails.map((item) => ({
        ...item,
        questionId: responsePattern[item.responseIndex]?.questionId,
      }));

      setGmatSectionScore(sectionScore);
      setGmatPercentile(percentile);

      // Calculate total time
      const totalTimeSeconds = Array.from(answers.values()).reduce((sum, a) => sum + a.timeSpent, 0);

      // Build per-question answers data
      ...
      // In preview mode, don't save results
      if (isPreviewMode) {
        const previewResult: GmatAssessmentResult = {
          ...
          metadata: {
            ...
            theta,
            adjusted_theta: adjustedTheta,
            se,
            unanswered_count: unansweredCount,
            percentile,
            cycle: studentCycle,
            algorithm_version: '1.1.0',
            response_pattern_details: responsePatternDetails,
          },
        };
      ...
      // Save result
      const savedResult = await saveSectionAssessmentResult(
        studentId,
        section,
        correctCount,
        questions.length,
  ...
  // Convert UnifiedAnswer back to stored format
  ...
  // Check if section is DI (for calculator)
  const isDISection = section === 'DI';
  ...
  // Determine back URL based on path
  const backUrl = window.location.pathname.startsWith('/tutor')
    ? '/tutor/students' // Tutors go back to student list
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading section assessment...</p>
        </div>
      </div>
    );
  }
  ...
  const unansweredDisplayCount = questions.length - answeredCount;
  ...
  if (testCompleted && result) {
    ...
    const wrongCount = questionResults.filter(r => !r.isCorrect && !r.isUnanswered).length;
    const unansweredResultCount = questionResults.filter(r => r.isUnanswered).length;

    // Pacing calculations
    const expectedTimePerQuestion = Math.round((sectionConfig!.timeMinutes * 60) / questions.length);
    ...
    return (
      <Layout
        pageTitle={isPreviewMode ? 'Preview Complete' : 'Section Assessment Complete'}
        pageSubtitle={sectionConfig?.fullName || section}
      >
        <MathJaxProvider>
          <div className="flex-1 p-4 md:p-8">
            <div className="max-w-5xl mx-auto">
              ...
              <div className="bg-white rounded-2xl shadow-lg border-2 border-gray-100 p-6 mb-6">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h1 className="text-2xl font-bold text-gray-800">
                      {sectionConfig?.fullName} Assessment Complete!
                    </h1>
                ...
                {/* GMAT Score + Stats Summary */}
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                  {/* GMAT Section Score - Prominent */}
                  <div className="col-span-2 md:col-span-1 text-center p-4 bg-indigo-50 rounded-xl border-2 border-indigo-200">
                    <div className="text-4xl font-bold text-indigo-600">
                      {gmatSectionScore ?? '-'}
                    </div>
                    <div className="text-sm text-indigo-700 font-medium">GMAT Score</div>
                    <div className="text-xs text-gray-500 mt-1">60-90 scale</div>
                  </div>
                  <div className="text-center p-4 bg-purple-50 rounded-xl">
                    <div className="text-3xl font-bold text-purple-600">
                      {gmatPercentile != null ? `${gmatPercentile}%` : '-'}
                    </div>
                  ...
                  <div className="text-center p-4 bg-amber-50 rounded-xl">
                    <div className="text-3xl font-bold text-amber-600">
                      {result.time_spent_seconds ? `${Math.floor(result.time_spent_seconds / 60)}m` : '-'}
                    </div>
                ...
                {/* Unanswered notice */}
                {unansweredResultCount > 0 && (
                  <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
                    <FontAwesomeIcon icon={faExclamationTriangle} className="mr-2" />
                    {unansweredResultCount} question{unansweredResultCount > 1 ? 's were' : ' was'} left unanswered (significant score penalty applied).
                  </div>
                )}
                            ...
                            return (
                              <div
                                ...
                                className={`p-3 rounded-lg border ${
                                  qr.isUnanswered ? 'bg-gray-50 border-gray-200' :
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <FontAwesomeIcon
                                      icon={qr.isUnanswered ? faExclamationTriangle : qr.isCorrect ? faCheckCircle : faTimesCircle}
                                      className={qr.isUnanswered ? 'text-gray-400' : qr.isCorrect ? 'text-green-600' : 'text-red-600'}
                                    />
                                    {qr.isBookmarked && (
                                ...
                                <div className="text-xs text-gray-500 mt-1">
                                  ...
                                  {qr.isUnanswered && <span className="ml-1 text-red-500">(unanswered)</span>}
                                </div>
                                ...
                                return (
                                  <div key={qr.question.id} className="flex-1 min-w-[20px] max-w-[40px] flex flex-col items-center relative group">
                                    ...
                                    <div className="absolute bottom-full mb-1 hidden group-hover:block bg-gray-800 text-white text-xs rounded px-2 py-1 whitespace-nowrap z-20">
                                      Q{i + 1}: {qr.timeSpentSeconds}s {qr.isUnanswered ? '?' : qr.isCorrect ? '✓' : '✗'}
                                    </div>
                                  </div>
                                );
                              })}
                    ...
                    <div className="flex flex-wrap items-center gap-4 mb-4 pb-4 border-b border-gray-100">
                      <div className="flex items-center gap-1">
                        ...
                        >
                          ...
                          {wrongCount + unansweredResultCount}
                        </button>
                      </div>
                      {bookmarkedCount > 0 && (
                        <button
                      ...
                      {filteredQuestions.map((qr) => (
                        <div
                          ...
                          className={`rounded-2xl border-2 overflow-hidden ${
                            qr.isUnanswered ? 'border-gray-200' :
                            qr.isCorrect ? 'border-green-200' : 'border-red-200'
                          }`}
                        >
                          <div className={`px-6 py-4 border-b flex items-center justify-between ${
                            qr.isUnanswered ? 'bg-gray-50 border-gray-100' :
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
        ...
        <TimeUpModal
          ...
          unansweredCount={unansweredDisplayCount}
        ...
        <div className="max-w-2xl w-full">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <div className="text-center mb-8">
              <h1 className="text-2xl font-bold text-gray-800 mb-2">Section Assessment</h1>
              <p className="text-gray-600">{sectionConfig?.fullName}</p>
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                ...
                <span className="font-bold text-gray-800">{sectionConfig?.timeMinutes} minutes</span>
              </div>
              <div className="flex items-center justify-between p-4 bg-indigo-50 rounded-xl">
                <span className="text-indigo-700">Scoring</span>
                <span className="font-medium text-indigo-800 text-sm">GMAT Section Score (60-90)</span>
              </div>
  ...
  // Review Phase UI
  if (inReviewPhase) {
    const reviewFilteredQuestions = reviewFilter === 'bookmarked'
        ...
        <div className="h-screen bg-gray-100 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-auto p-4">
            <div className={`${hasPassage ? 'max-w-7xl' : 'max-w-4xl'} mx-auto`}>
              ...
              {reviewFilteredQuestions.length === 0 && reviewFilter === 'bookmarked' && (
